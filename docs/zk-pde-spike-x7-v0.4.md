...
**γ 正确取值范围（修正）：**
- imbalance_ratio = 0（只进不出）→ γ = 0.2（最小衰减，势位保留最久）
- imbalance_ratio = 1（互引均衡）→ γ = 0.3（正常衰减）
- imbalance_ratio → ∞（只出不进）→ γ → ∞（最大衰减，饱和截断）

> ⚠️ 修正：原文档"取值范围 0.3 ~ 0.7"有误。正确范围是 0.2 ~ ∞，0.7 是笔误。

**γ 行为分析（修正）：**
| 状态 | imbalance_ratio | γ 值 | 含义 |
|------|-----------------|------|------|
| 只进不出（只被引用不引用） | → 0 | γ → 0.2 | 势位保留最久 |
| 互引均衡（互相引用） | ≈ 1 | γ = 0.3 | 正常衰减 |
| 只出不进（只引用不被引用） | → ∞ | γ → ∞（截断） | 势位衰减最快 |

> ⚠️ 修正 P1：原表"只进不出"标签与含义不符。改为"只进不出"对应 imbalance_ratio→0（接收多给出少），"只出不进"对应 imbalance_ratio→∞（给出多接收少）。

**物理含义：** 互惠均衡者（互相引用）活得久；只出不进者（只引用不被引用）衰减快。这与 ECHO "势位自然形成/消散"的道统一致。
...
| `g_min` | uint256[] | 每节点最小增长因子下界（公开参数） |
| `g_max` | uint256[] | 每节点最大增长因子上界（公开参数） |
| `step_index` | uint256 | 当前步数索引（防止重放） |
| `phi_max` | uint256/Q32.32 | 历史最大势位（用于 γ 公式分母） |

**PublicInputs 字段共 11-12 个（Seaman_bot v0.4.2 接口规范），以 Seaman_bot 最终 schema 为准，此处补充待最终对齐。**
