# X7 PDE 滑动窗口框架 v0.3
**更新日期：** 2026-06-19
**更新内容：** v0.2 → v0.3，补充四权→ξ 合成函数（雨娃拍板）
...
**四权 → ξ 合成函数（v0.3 新增，雨娃 2026-06-19 拍板）：**
```
ξ = (D + A + K + P) / 4
```
...
**说明：**
- 四权等权平均，输出范围 0-5，与六档一致
- α 系数终值待蒙特卡洛数据回填后替换占位值 0.25×4
- 映射规范 v0.3 接口空白是 design intent，不是遗漏
...
- **v0.2（作废）：** S_base = ξ/5.0 已定义，但四权→ξ 合成函数未落文字
- **v0.3（当前）：** 补充四权→ξ 合成函数 ξ=(D+A+K+P)/4，雨娃 2026-06-19 拍板
