"""
ECHO 离散事件驱动模型 E0-E8 实验
=========================================
【重要】这是离散事件驱动模型，不是 PDE 实现！

用途：
- 快速参数扫描（2分钟 vs PDE 10+分钟）
- 发现主导参数（beta是主控，耦合形式差异小）
- 作为"参数搜索工具"，先跑出合适参数再填到PDE规范

特点：
- Gini量级0.003-0.03（与真实PDE差15-100倍，不能直接对标）
- 事件驱动：random choice + gain/loss
- 动态gamma：出度高的节点衰减更快
